...
// Virtual Turbo Boost
bool virtualTurbo = false;
unsigned long lastTurboMillis = 0;
const unsigned long turboDuration = 1000;
...
void loopIbus() {
  ...
  if (millis() - lastIbusRead > 10) {  // Every 10ms
    ...
    else ibusInit = true;  // We need to process the entire serial package, before we read the channels for the first time! 100 OK? TODO
  }
}
...
void readSumdCommands() {
  ...
  if (SUMD_init) {  // TODO, experimental!
    // Normalize, auto zero and reverse channels
    processRawChannels();
...
void processRawChannels() {
  ...
  if (millis() - printChannelMillis > 10000 && autoZeroDone) {  // Every 1000ms
...
#undef TRANSMISSION_NEUTRAL  // Not usable in this case!
  ...
  static uint16_t winchServoMicrosTarget = CH4C;
  static uint16_t winchServoMicros = CH4C;
...
void headLightsSub(bool head, bool fog, bool roof, bool park) {
  ...
  if (millis() - xenonMillis > 50) xenonIgnitionFlash = 0;
  else xenonIgnitionFlash = 170;  // bulb is brighter for 50ms
#endif
  ...
  if (headLightsFlasherOn || (headLightsHighBeamOn && head)) roofLight.pwm(200 - crankingDim);
  else roofLight.off();

#else   // Bulbs wired as labeled on the board ----
  // Headlights
  if (!head && !park) {  // OFF or flasher
    if (!headLightsFlasherOn) headLight.off();
    else headLight.on();
    xenonMillis = millis();
    headLightsHighBeamOn = false;
  } else if (park) {  // Parking lights
    if (!headLightsFlasherOn) headLight.pwm(constrain(headlightParkingBrightness - crankingDim, (headlightParkingBrightness / 2), 255));
    else headLight.on();
    xenonMillis = millis();
    headLightsHighBeamOn = false;
  } else {  //ON
  ...
  // Roof lights
  if (!roof) roofLight.off();
  else roofLight.pwm(130 - crankingDim);
#endif  // ----

  // Fog lights
  if (!fog) fogLight.off();
  else fogLight.pwm(200 - crankingDim);
}
...
void led() {
  ...
  if (millis() - flickerMillis > 30) {  // Every 30ms
    ...
    if (engineStart) crankingDim = random(25, 55);
    else crankingDim = 0;  // lights are dimmer and flickering while engine cranking
  }
#else
  if (engineStart) crankingDim = 50;
  else crankingDim = 0;  // lights are dimmer while engine cranking
#endif

  if (headLightsFlasherOn || headLightsHighBeamOn) dipDim = 10;
  else dipDim = 170;  // High / low beam and headlight flasher (SBUS CH5)
...
#if defined OVERDRIVE && defined VIRTUAL_3_SPEED && defined VIRTUAL_TURBO // Additional 4th gear mode for virtual 3 speed and virtual turbo boost *********************
  if (!crawlerMode) {
    ...
    // Virtual Turbo boost
    if (currentThrottle > 800 && overdrive && virtualTurbo && millis() - lastTurboMillis > 1000) {
      currentRpm += 500;
      lastTurboMillis = millis();
    }
  }
#endif                                       
// End of overdrive ******************************************************************************************************
  ...
  if (millis() - manualTransDebugMillis > 10000) {
...
void trailerControl() {
  ...
  if (millis() - espNowMillis > pollRate) {  // Every 20ms
    ...
    if (taillightOld != trailerData.tailLight || sidelightOld != trailerData.sideLight || reversingLightOld != trailerData.reversingLight || indicatorLOld != trailerData.indicatorL || indicatorROld != trailerData.indicatorR || legsUpOld != trailerData.legsUp || legsdownOld != trailerData.legsDown || rampsUpOld != trailerData.rampsUp || rampsdownOld != trailerData.rampsDown || beaconsOnOld != trailerData.beaconsOn) {
